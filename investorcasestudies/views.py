...
class InvestorCaseStudiesList(APIView):
    def get(self, request, *args, **kwargs):
        ...
        response = supabase.table("InvestorCaseStudies").select("*").execute()
...
def CreateInvestorCaseStudies(request):

    file_path = "Book5.xlsx"  # Replace with your file path
    ...
    for row in sheet.iter_rows(values_only=True): # Each row as a tuple
        supabase.table("InvestorCaseStudies").insert({
            "Investor": row[0],
            "Overview": row[1],
            "Key Takeaway": row[2],
            "Source": row[3],
