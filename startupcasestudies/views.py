...
class StartupCaseStudiesList(APIView):
    def get(self, request, *args, **kwargs):
        ...
        response = supabase.table("StartupCaseStudies").select("*").execute()
...
def CreateStartupCaseStudies(request):

    file_path = "Book4.xlsx"  # Replace with your file path
    ...
    for row in sheet.iter_rows(values_only=True): # Each row as a tuple
        supabase.table("StartupCaseStudies").insert({
            "Startup": row[0],
            "Overview": row[1],
            "Key Takeaway": row[2],
            "Source": row[3],
