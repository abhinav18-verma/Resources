...
class StartupToolsList(APIView):
    def get(self, request, *args, **kwargs):
        ...
        response = supabase.table("StartupTools").select("*").execute()
...
def CreateStartupTools(request):

    file_path = "Book3.xlsx"  # Replace with your file path
    ...
    for row in sheet.iter_rows(values_only=True): # Each row as a tuple
        supabase.table("StartupTools").insert({
            "Serial Number": row[0],
            "Title": row[1],
            "Description": row[2],
            "Link": row[3],
